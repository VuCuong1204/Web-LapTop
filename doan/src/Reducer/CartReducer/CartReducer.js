...
export const getListCartAction = (data) => async (dispatch) => {
    try {
        ...
        dispatch(putListCartAction(response.data.data.filter(i => i.status === "0")))
        const newList = response.data.data.filter(i => i.cartSelected === "1" && i.status === "0")
        dispatch(setCheckedAll(newList.length === response.data.data.filter(i => i.status === "0").length ? true : false))
