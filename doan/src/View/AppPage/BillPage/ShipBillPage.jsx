...
import { getListBillTraverUser, stateBillUser } from "../../../Reducer/BillReducer/BillReducer";
import { stateGlobal } from "../../../Reducer/GlobalReducer/GlobalReducer";
import { Link, useHistory } from "react-router-dom/cjs/react-router-dom.min";
import { Button, Empty } from "antd";

export default function ShipBillPage(props) {
    const dispatch = useDispatch();
    const { listBillTraverUser } = useSelector(stateBillUser)
    const { userInfo } = useSelector(stateGlobal)
    const history = useHistory();
    useEffect(() => {
        let data = new FormData();
        data.append("accountId", userInfo.id)
        dispatch(getListBillTraverUser(data));
    }, [])
    return (
        <>
            {
                listBillTraverUser.length === 0 ? (
                    <Empty
